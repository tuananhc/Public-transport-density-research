...
# function to get income for a postcode
...
for index, row in victoria_map.iterrows():
    if (not np.isnan(row["income"])) and (not np.isnan(row["avgDis"])):
        listCluster.append([row["income"], row["avgDis"]])
        xlist.append(row["income"])
...
plt.title("Scatter plot of income with distance to a " + sys.argv[1] + " stop")
plt.xlabel("Average income (AUD)")
