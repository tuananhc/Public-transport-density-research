...
victoria_map["populationDensity"] = np.log(victoria_map["population"]/victoria_map["area"])

# Plot the data and save figure
fig, ax = plt.subplots(1)
color = 'Blues'
vmin, vmax = victoria_map["populationDensity"].min(), victoria_map["populationDensity"].max()
sm = plt.cm.ScalarMappable(cmap=color, norm=plt.Normalize(vmin=vmin, vmax=vmax))
sm._A = []
cbar = fig.colorbar(sm,fraction=0.046, pad=0.04)
cbar.ax.tick_params(labelsize=8)
victoria_map.plot('populationDensity', cmap=color, ax=ax, edgecolor='0.8')
plt.title("Population density in logarit")
ax.axis('off')
